...
Deno.serve(async (req : Request) => {
    ...
    const bodyJson = await req.json(); /* bodyJson here also includes: notify-faculty and id <- meeting id */
    const body : BodyType = {
    ...
    const { data: updateMeetingData, error: updateMeetingError } = await supabaseClient.from('meetings')
        .update(body)
        .eq("id", bodyJson.id)
        .select(returnSelect)
        .returns<rtyp[]>();
